...
- 提供 `RayonTaskHandle`，支持阻塞 `get`、异步等待、启动前取消与完成状态检查。
...
`submit` 或 `submit_callable` 成功只表示任务已被服务接受。最终任务结果通过 `RayonTaskHandle` 报告。

队列中的任务在 Rayon 开始运行前可以被取消。`shutdown` 停止接受新任务，并允许已接受工作完成。`shutdown_now` 停止接受新任务，并取消尚未开始的工作；已经运行的 CPU 工作不会被强制停止。
